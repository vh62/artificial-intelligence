...
def visualize_3d(df, lin_reg_weights=[1,1,1],
                    feat1=0, feat2=1, labels=2, alpha=0.,
                    xlim=(-1, 1), ylim=(-1, 1), zlim=(0, 3),
                    title='', xlabel='age', ylabel='weight', zlabel='height'):
    """ 
    3D surface plot. 
    Main args:
      - df: dataframe with feat1, feat2, and labels
    ...
    ax.set_title(title or f"LinReg Height with Alpha {alpha}")
    plt.show()
...
if __name__ == "__main__":
    ...
    visualize_3d(df, lin_reg_weights=lin_reg_weights,
                    ...
                    xlabel=cols[0], ylabel=cols[1], zlabel=cols[2])
